...
def create_flight_map(source, destination):
    """Create a map showing the flight route"""
    source_coords = city_coords[source]
    dest_coords = city_coords[destination]
    
    # Create dataframe for cities
    cities_df = pd.DataFrame([
        {"city": source, "lat": source_coords[1], "lon": source_coords[0], "type": "source"},
        {"city": destination, "lat": dest_coords[1], "lon": dest_coords[0], "type": "destination"}
    ])
    
    # Create dataframe for flight path
    flight_path = pd.DataFrame([
        {"lat": source_coords[1], "lon": source_coords[0]},
        {"lat": dest_coords[1], "lon": dest_coords[0]}
    ])
    
    # Calculate map center and zoom
    center_lat = (source_coords[1] + dest_coords[1]) / 2
    center_lon = (source_coords[0] + dest_coords[0]) / 2
    
    # Calculate distance for zoom level
    lat_diff = abs(source_coords[1] - dest_coords[1])
    lon_diff = abs(source_coords[0] - dest_coords[0])
    max_diff = max(lat_diff, lon_diff)
    zoom = max(3, min(8, 8 - max_diff * 0.5))
...
with col1:
    ...
    # Display flight route map
    st.markdown('<div class="form-card fade-in-up"><div class="section-title">🗺️ Flight Route Map</div></div>', unsafe_allow_html=True)
